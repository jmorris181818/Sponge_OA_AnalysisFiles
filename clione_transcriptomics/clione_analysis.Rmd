...
```{r}
read.table(file = "~/Dropbox/PhD/people/john/clione/allcounts_host.txt",
           row.names = 1) -> counts

read.table(file = "~/Dropbox/PhD/people/john/clione/Varians_samples[1].txt") %>% 
  mutate(TREATMENT = as.factor(treatment), 
         COLONY = as.factor(colony)) -> tfall

str(tfall)
```

sample 9 not in the count file, removing from metadata

```{r , include = F}
read.table(file = "/Users/benyoung/Dropbox/PhD/people/john/clione/Cvarians-Gerakladium-denovo-transcriptome-main/Cvarians_iso2geneName.tab",
          sep = "\t") %>%
  mutate(GeneID = V1,
         Gene.Annotation = V2) %>%
  dplyr::select(-V1, -V2) %>%
  full_join(counts %>%
              ...
              dplyr::select(GeneID)) %>% 
  full_join(read.table("/Users/benyoung/Dropbox/PhD/people/john/clione/Cvarians-Gerakladium-denovo-transcriptome-main/Cvarians_iso2go.tab",
            sep = "\t") %>%
              mutate(GeneID = V1,
                     GOID = V2) %>%
              dplyr::select(-V1, -V2)) %>%
  full_join(read.table("/Users/benyoung/Dropbox/PhD/people/john/clione/Cvarians-Gerakladium-denovo-transcriptome-main/Cvarians_iso2kegg.tab",
            sep = "\t") %>%
              mutate(GeneID = V1,
                     KEGGID = V2) %>%
              dplyr::select(-V1, -V2)) %>%
  full_join(read.table("/Users/benyoung/Dropbox/PhD/people/john/clione/Cvarians-Gerakladium-denovo-transcriptome-main/Cvarians_iso2kogClass.tab",
            sep = "\t") %>%
              mutate(GeneID = V1,
                     KOG.annotation = V2) %>%
              dplyr::select(-V1, -V2)) %>%
  full_join(read.table("/Users/benyoung/Dropbox/PhD/people/john/clione/Cvarians-Gerakladium-denovo-transcriptome-main/Cvarians_seq2iso.tab",
            sep = "\t") %>%
              mutate(FastaID = V1,
                     GeneID = V2) %>%
              dplyr::select(-V1, -V2)) %>%
  mutate(across(everything(), ~ replace_na(.x, "-"))) -> clione_annot
View(clione_annot)

write.csv(clione_annot,
          file = "~/Dropbox/PhD/people/john/clione/clione_annot_r.csv")
```

### Fasta File of just reads in count matrix 

```{r reading in fasta and organising, include = F}
readDNAStringSet(filepath = "~/Dropbox/PhD/people/john/clione/Cvarians.fasta") %>%
  as.data.frame() %>%
  rownames_to_column(var = "FastaID") %>%
  dplyr::rename(., sequence = x) %>%
  mutate(genelength = str_length(sequence)) %>%#double checking length is same as in fasta, it is
  inner_join(clione_annot %>%
               dplyr::select(FastaID, GeneID)) %>%
  group_by(GeneID) %>% 
  slice_max(., genelength) %>%
  distinct(GeneID, .keep_all = T) -> clione_sequence_to_gene

# View(clione_sequence_to_gene)
write.csv(clione_sequence_to_gene, 
          file = "~/Dropbox/PhD/people/john/clione/clione_sequence_to_gene.csv")
...
```{r venn to see count and fast deiscrpenmcy, echo = T}
venn(list(rownames(counts), clione_sequence_to_gene$GeneID))
```

A few more in the filtered clione fasta file when summarised to transcript level than to the counts file. Probably just got excluded as had no counts ? Not really sure but do not think it matters to much. 

Using the venn it looks like 124 extra genes in the fasta file. Any ideas Michael?


```{r , include = F}
nrow(clione_sequence_to_gene)
nrow(counts)
```
...
#                           file = "~/Dropbox/PhD/people/john/clione/clionedf.fasta")
```
...
With the filtering (less than 8 counts in 1 sample) get us 23k genes. Pretty workable.  
...
nrow(filt_fasta_4_egg)
  
dataframe = data.frame(filt_fasta_4_egg$GeneID,
                       filt_fasta_4_egg$sequence)

df.fasta <- dataframe2fas(dataframe,
                          file = "~/Dropbox/PhD/people/john/clione/filt_fasta_4_egg.fasta")
```
...
```{r Plots from PCAtools, fig.width=12, fig.height=6, include = T}
...
Basically saying how many PC axes you need to include to get a good sense of the variance. 
...
You can see not much strong significane for the r2 correlations. Again colony is driving the gene expresion variance for clione. 
...
```{r ggplots with colony and treatment, echo = T}
...
```{r VSD with colony removed, include = F}
vsdgenet <- vst(ddsall)
...
```{r Plots from PCAtools, fig.width=12, fig.height=6, echo = T}
## Scree plot showing amount of variance explained by each PC (bars) and cumulative variance as you progress along bars (line)
screeplot(allsamps, 
          getComponents(allsamps, 1:15), 
          axisLabSize = 10, 
...
```{r EigenPlots, fig.width=13, fig.height = 7, echo = T}
# Plotting of the metadata to the PC axes to see which one has strong significant relationships with axes. 
eigencorplot(
  allsamps,
  components = getComponents(allsamps, 1:10),
...
No significant correlations of any variables for PC axes when removing the colony variance. 
...
```{r ggplot of PCA with variance removed, echo = T}
...
At alpha 0.001, decent amount of significant genes (See output from above). Around 9k genes. 
...
Sorts model alphabetical, so AMB will be base level which is what we want 
...
```{r}
clione_annot %>%
  dplyr::filter(FastaID %in% clione_sequence_to_gene$FastaID) %>%
  dplyr::mutate(test = str_replace_all(Gene.Annotation,
                                       pattern = "^-$",
                                       replacement = "hypothetical_protein")) -> clione_annot_GENES
write.csv(clione_annot_GENES %>%
            dplyr::filter(GeneID %in% rownames(OA_versus_AMB_res_0.05)), file = "~/Dropbox/PhD/people/john/clione/hm_genes_fix.csv")

read.csv(file = "~/Dropbox/PhD/people/john/clione/hm_genes_fix.csv") -> hm_fix_annots
```
...
#venn(list(rownames(matmatnorder), hm_fix_annots$GeneID))
...
henlo$Gene.Annotation <- as.character(henlo$Gene.Annotation)
# 
...
```{r PC1 Variance Heatmaps, fig.height=7, fig.width=5}
# NB manual specify of the GO columns, here it is 86:93. First heatmap we remove, 2nd heatmap we select. 
Heatmap(
  t(scale(
    t(henlo %>% as.data.frame() %>% dplyr::select(-Gene.Annotation))
  )),
  cluster_columns = T,
  show_row_dend = F,
  show_column_dend = T,
  ...
  right_annotation = rowAnnotation(foo = anno_text(
    henlo$Gene.Annotation,
    gp = gpar(fontsize = 5.5, font = 8)
  ))
)
